...
## About using docker_container and semi-mandatory quoting

`docker_container` (>=2.8) now insists that all environment values be quoted. Or it sometimes does, depending on rules as equally ambiguous as the original problem this was trying to solve. When evaluating `"{{ foo }}"`, those quotes are removed. So use the `to_json` jinja2 filter to ensure that the value is quoted. Note: I use `to_json` instead of `quote` because `quote` will not quote Boolean values `true` and `false`.

If not quoted, the error will look like `"Non-string value found for env option. Ambiguous env options must be wrapped in quotes to avoid them being interpreted. Key: ENV_VAR_NAME"`. If you see this error, randomly sprinkle `to_json` in your interpolations and try again.
